...
프론트엔드 프레임워크를 사용하는 현대적인 웹 개발에선 다크 모드를 어떻게 구현하면 될까? 앞서 CSS를 사용해 다크 모드를 구현하는 방식을 살펴 보았다. 그런데 다크 모드 기능은 조금 더 복잡해질 수 있다. 예를 들어, 다크 모드에 따라 스타일이 아닌 HTML 엘리먼트의 내용이 변경될 수도 있다. 이를 구현하기 위해선 컴포넌트에서 다크 모드에 대한 상태를 참조해 내용을 변경해야 한다. 따라서 프론트엔드 프레임워크를 사용하는 방식에선 주로 전역 상태를 사용해 다크 모드를 관리하게 된다.

이번 블로그 Next.js 기반 개편 작업에선 CSS-in-JS 라이브러리 <a href="https://styled-components.com/"><strong>styled-components</strong></a>와 전역 상태 관리 라이브러리 <a href="https://zustand-demo.pmnd.rs/"><strong>Zustand</strong></a>가 사용되었다.
