...
아니 CSS에서 사용하는 기호들, 점(.)이나 공백( )도 다 제거된 형태인데 왜 인식하지 못하는 것일까? 정상적으로 HTML 엘리먼트에 ID 속성으로 적용되는 평범한 문자열 처럼 보이는데. 여기서 함정이 있다. CSS 선택자는 HTML 엘리먼트의 ID나 클래스를 선택하는데 흔히 사용되지만, 그렇다고 CSS 선택자가 HTML의 ID 혹은 클래스와 완전히 동일한 개념은 아니다. HTML에는 HTML의 문법이 있고 CSS에는 CSS의 문법이 있는 법. HTML에서는 ID 속성의 값이 숫자로 시작해도 괜찮다. CSS 선택자는 숫자로 시작할 수 없다. 대부분의 프로그래밍 언어에서 변수 명은 숫자로 시작할 수 없는 것과 비슷한 이치일 것.

### 해결

아무튼 그러면 어쩐담. CSS 규칙 때문에 문단 제목에 숫자를 안 쓰는 것도 이상하고. 

그래서 이스케이프 처리를 해줘야 함
=> 이스케이프 처리란?
=> 그래서 이스케이프 처리를 하면 어떻게 결과가 나오는데?
=> 어떻게 사용하는데?
...
앞으로 querySelector를 사용할 때 CSS.escape를 고려해야겠다.
흔히 사용하는 함수이기도 하고 쿼리 셀렉터 자체가

그리고 요즘 테스팅에 내가 너무 소홀히 한 죗값을 톡톡히 치르고 있단 생각도 듦
