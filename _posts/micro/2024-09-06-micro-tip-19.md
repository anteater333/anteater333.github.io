...
<strong>사파리에선 스크롤 값이 음수가 될 수 있다</strong>. 위처럼 mdn web docs에서도 특별히 한 문단을 할애해 사파리의 특별한 동작에 대해 설명하고 있다. 사파리는 기본적으로 웹 페이지에 Elastic Overflow Scrolling 혹은 Rubber band 라고 불리는 효과를 적용한다. 간단히 말하면 움직이는 물체에 탄성을 준다는 것이다. 아래 애니메이션 처럼.
...
이 탄성은 움직이는 물체가 정지선에 닿는 순간 그 지점을 약간 넘어섰다가 다시 되돌아가는 것 처럼 보이게 만든다. 역시 미적 감각이 뛰어난 애플의 대표 브라우저 답게 이런 부분에 신경을 쓰는군. 실제 코드로 한 번 그 동작을 체험해보자.
...
웹 환경은 마치 JVM과 비슷한 시각으로 쓰이고 있다고도 느껴진다. 웹 페이지를 브라우저라는 가상 머신 위에서 돌아가는 어플리케이션이라고 볼 수 있지 않을까. 그만큼 집중해야 할 부분은, 내가 만들고 있는 이 웹 페이지는 정말 다양한 환경에서 돌아갈 것이라는 사실이다. 지금 내가 사용 중인 개발 PC, 내가 애용하는 브라우저에서만 잘 돌아간다고 이 웹 페이지가 잘 만들어졌다고 착각해선 곤란하다.

광범위한 지원 가능성을 갖춰야할 필요
...
https://velog.io/@eunbinn/js-dates-finally-fixed

우연히 js Date 객체가 변경될 수 있다는 글을 찾았다. 이것이 사파리의 Date 파싱 문제를 처리할 수 있을까 아니면 사파리의 뒤쳐진 표준화 문제에 더 짐이 될 뿐일까. 어차피 결과는 먼 미래의 일이겠지만.