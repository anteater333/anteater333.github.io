...
CSR과 SSR 둘 중 어떤 방식을 사용하던 기본적으로 사용자는 서버에 요청을 보내 웹 페이지를 받아야 한다. 기본적으로 웹 페이지를 구성하는 3대 요소를 HTML/CSS/JavaScript로 정리할 수 있지만, 현대 웹 개발에선 그 중 가장 열일하는 것이 자바스크립트가 되어버린 것 같다. 존재감이 너무 커지다보니 그 역할을 크게 두 부분으로 나눌 수도 있을 지경이다.
