...
- <strong>Hydration</strong>이란 이미 렌더링 된 마크업(HTML)에 화면의 동작 코드(JS)를 연결하는 과정이다.
...
그런데 밀키트의 포장지에 그려진 조리에와 고객이 조리한 결과가 너무 달라버리면 좀 곤란하겠지. 뉴스도 나고 소보원에서 나서고 책임자는 재판에 넘겨지고 가정 불화가 생기고 애는 굶고... 그렇기 때문에 리액트에선 하이드레이션 과정의 결과물이 서버에서 렌더링한 결과물과 너무 달라지는 상황, 즉 <strong>하이드레이션 불일치(Hydration Mismatch)</strong> 상황을 경고한다.
...
클라이언트측 코드라는 것을 명시하기 `'use client'` => 여전히 에러 뜨긴 함
...
- 근데 왜 SSG에서 이런 에러가 떴을까? (빌드 후에도 콘솔에 출력되는 에러 Minified React error #418)
  - SSG에도 하이드레이션 과정은 포함
https://i.postimg.cc/ZK5J6f6g/image.png
<strong>[참고자료들]</strong>
...
- https://velog.io/@tap_kim/ssr-deep-dive-for-react-developers