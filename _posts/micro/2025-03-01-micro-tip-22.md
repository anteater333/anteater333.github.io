...
조금 억지스럽게 느껴질 수도 있겠지만, 사용자가 완성된 웹 페이지를 이용하는 것을 식사에 비유해보자. 먼저 고전적인 웹 서버는 이미 다 만들어져 있는 완제품을 파는 편의점, WAS는 주문을 받았을 때 주방장이 요리를 시작하는 식당이라고 볼 수 있다.

이 비유를 현대적인 웹 페이지 구현 방식들에도 적용해본다면, CSR과 SSR은 사용자에게 재료와 레시피를 쥐어주고 직접 요리하길 요구하는 방식이라고 말할 수 있겠다. CSR은 사용자가 직접 원재료 손질부터 시작한다는 느낌이라면, SSR은 마치 밀키트를 구입하는 것과 비슷하다. 고기도 이미 양념에 재워져 있고 감자 껍질도 벗겨져있고 물만 붓고 끓이기만 하면 된다. 이것도 어쨌든 요리긴 하지. SSR에서 그 물을 붓는 행위를 바로 하이드레이션(Hydration, 수분공급)이라고 부른다.

인터렉션을 식사 행위로 표현한다면

과거의 웹은 가게가 요리한 음식을 배달하는 것
CSR은 원재료를 사서 직접 요리하는 것
SSR은 밀키트 (결국 본인이 요리하긴 함 === 하이드레이션)

기본적으로 웹 페이지를 구성하는 3대 요소를 HTML/CSS/JavaScript로 정리할 수 있지만, 현대 웹 개발에선 그 중 가장 열일하는 것이 자바스크립트가 되어버린 것 같다. 존재감이 너무 커지다보니 그 역할을 크게 두 부분으로 나눌 수도 있을 지경이다.
