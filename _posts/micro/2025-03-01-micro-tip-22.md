...
우선 CSR과 SSR 둘 중 어떤 방식을 사용하던 사용자가 서버에 요청을 보내 웹 페이지의 소스를 받아온다는 개념은 변하지 않는다. 그 소스엔 웹 페이지를 구성하는 3대 요소인 구조(HTML), 스타일(CSS), 동작(JavaScript)가 모두 들어있을 것이다. 다만 여기서 자바스크립트가 담당하는 동작이란 부분이 시대를 거듭하며 계속 커지다 못해, 동작의 결과로 다른 두 요소를 만드는 지경 까지 왔다. 그것이 바로 클라이언트 측 렌더링(CSR)이라고 볼 수 있다.
...
CSR이 식재료를 사와서 직접 요리하는거라면
SSR은 밀키트 정도의 난이도 차이
=> 물 끓이고 재료 넣고 소스 넣는 과정 === 하이드레이션
