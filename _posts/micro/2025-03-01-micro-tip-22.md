...
SSR은 위 두 과정 중 렌더링만 서버에서 담당한다. SSR 서버에 접속한 클라이언트는 동작이 정의되어있지 않은 매마른 웹 페이지의 최초 소스를 받게된다. 따라서 웹 페이지를 "동적으로", 촉촉하게 만든다는 의미에서 웹 페이지에 동작을 연결하는 두 번째 과정을 하이드레이션이라고 부르는 것이다.
