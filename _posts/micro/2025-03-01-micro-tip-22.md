...
글 신나게 썼는데 구글 네이버 다음 등등 검색 서비스들에게 외면당하면 안되잖아. 따라서 블로그 개발에는 서버에서 먼저 웹 크롤러도 인식할 수 있는 형태로 1차적인 렌더링을 해주는 방식, <strong>SSR(Server Side Rendering)</strong>을 지원하는 프레임워크 Next.js를 사용했다. 이렇게 웹 어플리케이션의 구현을 CSR과 SSR이라는 큰 두 방식의 분류로 나눌 수 있겠는데, 사실 엄밀히 따지면 하나를 쓴다고 다른 한 쪽은 완전히 사장되는 것은 아니긴 하다. 두 방식은 서로 얼마나 다르고, 어떻게 융화되는지에 대해서 알아둘 필요가 있다.
