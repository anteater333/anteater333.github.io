...
어떤 직업이든 간에 저마다 수족처럼 다루게 되는 도구가 있기 마련이다. 흔히들 머리를 쓰는 직업으로 여겨지는 소프트웨어 개발자도 예외는 아니다. 멀리갈 것도 없이 일단 컴퓨터가 있어야 하고 거기에 최소 키보드, 마우스, 모니터는 연결되어 있어야 사람처럼 일할 수 있다. 이런 하드웨어들을 물리적인 도구라고 표현했을 때 개발자에겐 논리적인 도구, 소프트웨어도 필요하다. 코드를 작성할 에디터, 실행할 컴파일러/빌더/인터프리터/기타등등... 그리고 그 결과를 확인할 디버거.

그렇다면 조금 더 상황을 한정지어, 웹 프론트엔드 개발자에게 있어 필요한 도구는 무엇일까?
