...
웹 개발자라면 자신이 짠 코드가 잘 돌아가는지 확인해보기 위해 콘솔 로그(`console.log`)를 로직 중간에 삽입하고, 브라우저로 가서 개발자 도구를 열어 원하는 순간에 로깅이 되는지를 확인하는 식으로 개발을 진행할 것이다. 이 버튼이 클릭이 먹히는지, 내가 원하는 조건문의 분기가 동작하는 지, API 호출이 제대로 완료되었는지...
