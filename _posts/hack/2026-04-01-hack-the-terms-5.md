...
첫 번째로 소개할 기능은 <strong>"요소(Elements)"</strong> 탭이다. 요소 탭은 기본 설정 기준 가장 첫 번째로 노출되는 탭으로, 만약 프론트엔드 개발을 한다면 가장 자주 찾게 되는 주무기 같은 기능이 될 것이다. 이때 요소(Elements)란 현재 문서를 구성하는 각각의 일부분들을 의미한다. 웹 페이지 형태로 제공되는 현재 글도 요소들로 구성되어 있으며, 이 단락도 요소, 바로 위에 첨부된 이미지도 요소, 화면 오른쪽 하단에 있는 버튼들도 요소라고 볼 수 있다.

요소 탭에서 보여주는 결과물은 페이지를 구성하는 HTML 코드 형태로 나타나며, <a href="/hack/4/rendering-in-web#중요-렌더링-경로critical-rendering-path">HTML을 해석해 도출한 DOM</a>을 확인하고 조작하는 기능을 제공하는 영역이라고 볼 수 있다. 따라서 아래와 같은 동작이 가능하다.
...
- 글의 의의?
- 개발자 도구는 워낙 광범위한 도구라서 모든걸 글 하나에서 설명할 순 없음
- 다만, 도구의 쓰임새는 다루는 사람에게 달려있다는 것.
- 소개한 기능 외에도 다양한 기능들이 있고, (개발자도구에서 아무거나 붙잡고 마우스 오른쪽 클릭이라도 해보시라)
- 여기선 어떻게 해야 어떤 기능이 튀어나오는지 궁금해 하는 것도 개발자의 몫
- 그렇게 알아낸 기능들을 어떻게 조합해 어떤 결과물을 낼지 고민하는 것도 개발자의 몫
- 약간 스위스 아미 나이프 같은 느낌...